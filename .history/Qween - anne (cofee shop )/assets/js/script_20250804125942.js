...
let delayCounter = 0; // single delay index across all sentences

tempWrapper.querySelectorAll('p').forEach((p) => {
  const paraClass = p.className;
  const paraContainer = document.createElement('p');
  paraContainer.className = paraClass;

  const text = p.innerHTML;
  const sentences = text.match(/[^.!?]+[.!?]+[\s]?|[^.!?]+$/g); // Split into sentences
  ...
  if (sentences) {
    sentences.forEach((sentence) => {
      ...
      delayCounter++; // increment for global stagger
    });
  }
