...
ScrollTrigger.create({
  ...
  onUpdate: (self) => {
    ...
    // Map velocity to a speedScale between 0.3 (slow) and 1 (normal)
    // Use an exponential decay for smoother transition
    const maxVelocity = 600; // tweak this value to control sensitivity
    const speedScale = gsap.utils.clamp(
      0.3,
      1,
      1 - (scrollVelocity / maxVelocity)
    );

    scrollTween.timeScale(speedScale);
  }
});
